...
export class Normal extends Distribution {
    pdf(arg: { x: number, mean: number, sd: number }) {
        let { x, mean, sd } = arg
        let d = x - mean
        return 1 / (sqrt(2 * PI) * sd) * Math.exp(-(d * d) / (2 * sd * sd))
    }
    cdf(arg: { x: number, mean: number, sd: number }) {
        let { x, mean, sd } = arg
        return 0.5 * (1 + P.erf((x - mean) / sqrt(2 * sd * sd)))
    }
    inv(arg: { p: number, mean: number, sd: number }) {
        let { p, mean, sd } = arg
        return -1.41421356237309505 * sd * P.erfcinv(2 * p) + mean
    }
    mean(arg: { mean: number, sd: number }) {
        let { mean, sd } = arg
        return mean
    }
    variance(arg: { mean: number, sd: number }) {
        let { mean, sd } = arg
        return sd * sd
    }
}
