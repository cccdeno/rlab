...
Deno.test("matrix: diag", () => {
  let D = M.diag([1,2,3])
  T.eq(M.flatten(D), [1,0,0, 0,2,0, 0,0,3])
})

Deno.test("matrix: identity", () => {
  let I = M.identity(3)
  T.eq(M.flatten(I), [1,0,0, 0,1,0, 0,0,1])
})
...
Deno.test("matrix: inv", () => {
  let b = M.inv(a)
  let ab = M.dot(a, b)
  T.assert(V.near(M.flatten(ab), [1,0, 0,1]))
})
...
Deno.test("matrix: LU", () => {
  let lup = M.lu(a)
  // console.log('lup=', lup)
  T.assert(V.near(M.flatten(lup.LU), [3, 4, 0.3333, 0.6667]))
  let b = [17, 39]
  let x = [5, 6]
...
Deno.test("matrix: SVD", () => {
  ...
  // console.log('UtU=', UtU)
  // console.log('VVt=', VVt)
  T.assert(V.near(M.flatten(UtU), M.flatten(M.identity(svd.U.length))))
  T.assert(V.near(M.flatten(VVt), M.flatten(M.identity(svd.V.length))))
  let US = M.dot(svd.U, M.diag(svd.S))
  let USVt = M.dot(US, Vt)
  // console.log('USV=', USVt)
  T.assert(V.near(M.flatten(USVt), M.flatten(a)))
})
